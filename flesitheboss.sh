						...
						case $apacheopc in
							...
							clear
								apt-get install postfix
									echo -n "Escriba su zona (flesitheboss.com): "
										read zona
											echo "mydomain = $zona"	>> /etc/postfix/main.cfg
									
									echo -n "Escriba el nombre de este equipo (server): "
										read equipo
											echo "myhostname = $equipo"	>> /etc/postfix/main.cfg
											echo "myorigin = $equipo"	>> /etc/postfix/main.cfg
											echo "mydestination = \$mydomain\, \$myhostname\ "	>> /etc/postfix/main.cfg
											#NUMERO DE REDES A AÑADIR...
									echo -n "Escriba el nombre de este equipo (server): "
										read equipo
											echo "myhostname = $equipo"	>> /etc/postfix/main.cfg
								...
								#MODIFICAR /etc/dovecot/conf.d/10-auth.conf
											...
											clear
												...
												echo -n "Selecciona una opcion: "
													...
													case $serperfilfijo in
														...
														3) # LDAP - INSTALAR PHPLDAPADMIN Y CONFIGRAR ACCESO VIA WEB
														clear
															...
															echo "Configurar el acceso vía web en fichero /usr/share/phpldapadmin/config/config.php"
																echo -n "DC [com]"
																	read dc0
																echo -n "DC [flesitheboss]"
																	read dc1
																
																#BORRAR LINEA 300
																	sed '300 d'
																	 # s/SetValue(‘server’,’base’,array(‘dc=example,dc=com’));/SetValue(‘login,’bind_id’,'cn=admin,dc=$dc1,dc=$dc0'));/g' /usr/share/phpldapadmin/config/config.php
																...
																# BORRAR LOS READONLY CON SED
																	sed -e '59 d' /etc/phpldapadmin/templates/creation/posixAccount.xml
																	sed -e '28 d' /etc/phpldapadmin/templates/creation/posixGroup.xml
														;; # LDAP - INSTALAR PHPLDAPADMIN Y CONFIGRAR ACCESO VIA WEB
													esac
									...
									echo -n "Selecciona una opcion: "
										read ldapopc
											case $ldapopc in
												...
												clear
												sed -i '1i session required pam_mkhomedir.so skel=/etc/skel umask=0022' /etc/pam.d/common-session
											...
											case $ldapopc in
												...
												clear
													echo -n "Escriba la IP del server: "
														read ipservernfs
															echo "$ipservernfs:/home/users /home nfs defaults 0 0   "   >> /etc/fstab
												;; # FIN LDAP - nfsserver
											esac
													...
													case $serperfilfijo in
														...
														clear
															...
															echo "Configurar el acceso vía web en fichero /usr/share/phpldapadmin/config/config.php"
																...
																# MODIFICAR LOS READONLY CON SED
